...
export const changeApproval = createAsyncThunk("wrapping/changeApproval", async ({ provider, address, networkID }: IChangeApproval, { dispatch }) => {
    ...
    const sOracleContract = new ethers.Contract(addresses.sORCL_ADDRESS, wMetisTokenContract, signer);

    let approveTx;
    try {
        const gasPrice = await getGasPrice(provider);

        approveTx = await sOracleContract.approve(addresses.wsORCL_ADDRESS, ethers.constants.MaxUint256, { gasPrice });
    ...
    const wsORCLAllowance = await sOracleContract.allowance(address, addresses.wsORCL_ADDRESS);

    return dispatch(
        fetchAccountSuccess({
            wrapping: {
                wsORCL: Number(wsORCLAllowance),
            },
        }),
    );
});
...
export const changeWrap = createAsyncThunk("wrapping/changeWrap", async ({ isWrap, value, provider, networkID, address }: IChangeWrap, { dispatch }) => {
    ...
    const wsOracleContract = new ethers.Contract(addresses.wsORCL_ADDRESS, wMetisTokenContract, signer);
    ...
    try {
        ...
        if (isWrap) {
            wrapTx = await wsOracleContract.wrap(amountInWei, { gasPrice });
        } else {
            wrapTx = await wsOracleContract.unwrap(amountInWei, { gasPrice });
        }
...
const calcWrapValue = async ({ isWrap, value, provider, networkID }: IWrapDetails): Promise<number> => {
    ...
    if (isWrap) {
        const wsORCLValue = await wsOracleContract.sORCLTowsORCL(amountInWei);
        wrapValue = wsORCLValue / Math.pow(10, 18);
    } else {
        const sORCLValue = await wsOracleContract.wsORCLTosORCL(amountInWei);
        wrapValue = sORCLValue / Math.pow(10, 9);
    }
