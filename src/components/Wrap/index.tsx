...
function Wrap({ open, handleClose }: IAdvancedSettingsProps) {
    ...
    const wsOracleBalance = useSelector<IReduxState, string>(state => {
        return state.account.balances && state.account.balances.wsORCL;
    });
    ...
    const sORCLAllowance = useSelector<IReduxState, number>(state => {
        return state.account.wrapping && state.account.wrapping.sORCL;
    });
    ...
    const setMax = () => {
        if (isWrap) {
            setValue(sOracleBalance);
        } else {
            setValue(wsOracleBalance);
        }
    };
    ...
    const hasAllowance = useCallback(() => sORCLAllowance > 0, [sORCLAllowance]);

    const trimmedsOracleBalance = trim(Number(sOracleBalance), 6);
    const trimmedWsOracleBalance = trim(Number(wsOracleBalance), 6);

    const getBalance = () => (isWrap ? `${trimmedsOracleBalance} sORCL` : `${trimmedWsOracleBalance} wsORCL`);
            ...
            <Paper className="ohm-card ohm-popover wrap-token-poper">
                <div className="cross-wrap wrap-cros-wrap">
                    ...
                    <div className="wrap-price" onClick={() => setIsWrapPrice(!isWrapPrice)}>
                        <p>
                            1 {isWrapPrice ? "sORCL" : "wsORCL"} = {`${trim(wrapPrice, 4)} ${isWrapPrice ? "wsORCL" : "sORCL"}`}
                        </p>
                    </div>
                </div>
