...
import { calcHyperBondDetails, changeApproval, hyperbondAsset } from "../../store/slices/hyperbond_slice";
...
function BondPurchase({ bond, slippage }: IBondPurchaseProps) {
    ...
    const vestingPeriod = () => {
        return prettifySeconds(bond.vestingTerm, "hours");
    };
    ...
    useEffect(() => {
        dispatch(calcHyperBondDetails({ bond, value: quantity, provider, networkID: chainID }));
    }, [bondDetailsDebounce]);
    ...
    //     dispatch(calcHyperBondDetails({ bond, value: "0", provider, networkID: chainID }));
    //     setZapinOpen(true);
    // };
    //
                ...
                {!hasAllowance() && !useAvax && (
                    <div className="help-text">
                        <p className="help-text-desc">
                            Note: The "Approve" transaction is only needed when bonding for the first time; subsequent bonding only requires you to perform the "Mint" transaction.
                        </p>
                    </div>
                )}
            </Box>
            ...
            <Slide direction="left" in={true} mountOnEnter unmountOnExit {...{ timeout: 533 }}>
                <Box className="bond-data">
                    ...
                    <div className={`data-row`}>
                        ...
                        <p className="price-data bond-balance-title">{isBondLoading ? <Skeleton width="100px" /> : `${trim(bond.maxBondPrice, 4)} ORCL`}</p>
                    </div>
